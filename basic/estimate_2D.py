...
import numpy as np
import torch
from utils_2D.estimate_pre_utils import split_prepare_data
from utils_2D.estimate_utils import train_val, predict
...
for seed in seed_values:
    ...
    results, results_B = train_val(seed, Y_train, X_train, Y_val, X_val, center_B_train, center_B_val, lr_init, epoch_init, step, lambda1, lambda2, tol, checkpoint_path, option="new")
    ...
    Y_test_pred = predict(Y_test, X_test, sigma_1, sigma_2, base, center_B_test, sigma_1_B, sigma_2_B, base_B)
    ...
    np.savez(data_path + '/' + filename, 
             ...
             log_sigma_1_B=log_sigma_1_B, log_sigma_2_B=log_sigma_2_B, base_B=base_B)
