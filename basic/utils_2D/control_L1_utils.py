...
def center_to_X(center_B, I1, I2, seed):
    ...
    for point in center_B:
        x, y = point
        X[int(x), int(y)] = np.random.uniform(-10, 10)
...
def best_center_alpha(alpha_min, alpha_max, numControl, max_iterations, tarAY, B_L1, I1, I2, X, t, seed):
    ...
    for i in range(max_iterations):
        ...
        lasso = Lasso(alpha=alpha/(2*(I1*I2)), fit_intercept=False, tol=1e-4, max_iter=5000, random_state=seed)
...
# verify the alpha path
def onlineControl_dymL1_B_alpha(Y_0, Np, ratio, noise_std, numControl, center_B, log_sigma_1, log_sigma_2, base, log_sigma_1_B, log_sigma_2_B, base_B, A, b_B, magnitude_B, target, seed):
    ...
    X[0] = center_to_X(center_B, I1, I2, seed)
    
    B_L1 = B_set(I1, I2, xx, yy, sigma_1_B, sigma_2_B, base_B)
    ...
    Y_tmp_wc = np.sum(A * Y_phys_wc[t-1], axis=(2, 3))
    tmp_Y = np.sum(A_est * Y_phys_wc[t-1], axis=(2, 3))
    ...
    for i in range(max_iter):
        ...
        center_B = X_to_center(X[t], non_zero_count)
        
        # determine control actions
        B_est = np.zeros((I1, I2, non_zero_count))
        for act in range(non_zero_count):
            B_est[:,:,act] = gaussian_product_kernel(xx, yy, center_B[act,0], center_B[act,1], sigma_1_B, sigma_2_B).T * base_B        
        ...
        Y_phys_wc[t] = Y_tmp_wc + XB_phys + np.random.normal(0, noise_std, (I1, I2)) * ratio
        ...
        iteration_info[i] = {
            ...
            'alpha_min': alpha_min,
            'alpha_max': alpha_max,
            'frobenius_norm_X': np.linalg.norm(X[t], 'fro'),
            'frobenius_norm_Y_pred_minus_target': np.linalg.norm(Y_pred[t] - target, 'fro'),
...
def onlineControl_dymL1_B(Y_0, Np, ratio, noise_std, numControl, center_B, log_sigma_1, log_sigma_2, base, log_sigma_1_B, log_sigma_2_B, base_B, A, b_B, magnitude_B, target, seed):
    ...
    for t in range(1, Np):
        ...
        Y_tmp_pred = np.sum(A_est * Y_pred[t-1], axis=(2, 3))
        Y_tmp_wc = np.sum(A * Y_phys_wc[t-1], axis=(2, 3))
        Y_tmp_nc = np.sum(A * Y_phys_nc[t-1], axis=(2, 3))
        tmp_X = np.sum(A_est * Y_phys_wc[t-1], axis=(2, 3))
        
        # determine control center
        tarAY = target - tmp_X
        alpha_max = cal_alpha_max(B_L1, tarAY, I1, I2)
        X, alpha = best_center_alpha(0, alpha_max, numControl, max_iter, tarAY, B_L1, I1, I2, X, t, seed)
        ...
        Y_pred[t] = Y_tmp_pred + XB_pred
        Y_phys_wc[t] = Y_tmp_wc + XB_phys + np.random.normal(0, noise_std, (I1, I2)) * ratio
        Y_phys_nc[t] = Y_tmp_nc + np.random.normal(0, noise_std, (I1, I2)) * ratio
