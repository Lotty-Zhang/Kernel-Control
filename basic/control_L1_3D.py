...
"""
import numpy as np
from utils_3D.init_location_utils import fix_sampling_3D
from utils_3D.control_repeat_utils import control_repeat_seed_L1
...
log_sigma_3 = data_control['log_sigma_3']
base = data_control['base']
log_sigma_1_B = data_control['log_sigma_1_B']
log_sigma_2_B = data_control['log_sigma_2_B']
log_sigma_3_B = data_control['log_sigma_3_B']
base_B = data_control['base_B']

#%% initial control locations
numControl = 6 # 3, 4, 5, 6
center_B = fix_sampling_3D(numControl, I1, I2, I3)

#%% repeat experiments
seeds = list(range(10))

# Conduct experiments and collect RMSD values
RMSD_values = [control_repeat_seed_L1(seed, Np, ratio, noise_std, data_path, I1, I2, I3, Y_0, numControl, center_B, log_sigma_1, log_sigma_2, log_sigma_3, base, log_sigma_1_B, log_sigma_2_B, log_sigma_3_B, base_B, A, b_B, magnitude_B, target) for seed in seeds]
...
print(f'3D L1, Mean RMSD: {mean_RMSD:.3f}')
print(f'3D L1, Variance RMSD: {var_RMSD:.2e}')
